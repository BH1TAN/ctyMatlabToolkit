function [] = decayanalysis(specs,t,pks,energyList)
% 分析序列测量的多列能谱各峰的衰变情况
% Inputs:
%    specs:多列矩阵，每一列是一个能谱
%    t：行向量，测量的起始时刻(s)
...
% Outputs:
%    "decayAnalysis"文件夹：计算生成的文件夹，包括
%    "decayAnalysis.mat"数据库文件，所有使用到的参数和结果
dirname = 'decayAnalysis';
mkdir(dirname);
roi = round([pks-0.01*pks,pks+0.01*pks]); % 0.01是个经验参数，大致将FWTM覆盖
...
for i = 1:size(roi,1)
    for j = 1:size(specs,2)
        ...
        clear h;
        [h,mat_gauss(i,j),~,~,~,~] = fitPeak(roi(i,1):roi(i,2),specs(roi(i,1):roi(i,2),j),0);
