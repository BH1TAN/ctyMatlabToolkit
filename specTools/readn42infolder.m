function [orgnSpec,t,t_real,t_live] = readn42infolder(foldername)
% 读取ortec循环存储的多个spe能谱并作图、保存
%
dir1 = dir([foldername,'\*.n42']);
...
tic;
for i = 1:length(dir1)
    disp(['Reading spectrum:',num2str(i),'/',num2str(length(dir1))]);
    disp(['Reading will finish in ',num2str(toc/i*(length(dir1)-i)),' second']);
    s = readn42([foldername,'\',dir1(i).name]);
